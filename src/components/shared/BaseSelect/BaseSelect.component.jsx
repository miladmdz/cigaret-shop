  ...
  return (
    <div className="relative select-none w-full" ref={dropdownRef}>
      <div
        ...
        className="w-full flex items-end justify-between px-4 py-2 border border-gray-300 rounded-2xl cursor-pointer"
      >
      ...
      {isOpenDropdown && (
        <ul className="absolute bg-white z-10 w-full rounded-2xl mt-1 px-2 py-4 border border-gray-300">
          {sortListState.map((item) => (
            ...
            >
              {item.label}
              {item.iconName && <IconLoader name={item.iconName} size={16} />}
            </li>
          ))}
        </ul>
      )}
