        ...
        <div className="flex mt-2 sm:mt-0 flex-col items-end gap-2">
          ...
          <div className="flex items-center justify-center">
            {Array.from({ length: 5 - rate }).map((_, i) => (
              <IconLoader name="star-solid" key={i} color="#9ca3af" size={15} />
            ))}

            {Array.from({ length: rate }).map((_, i) => (
              <IconLoader name="star-solid" key={i} color="#fa8439" size={15} />
            ))}
          </div>
        </div>

        <div className="relative w-[130px] sm:w-[180px] h-[80px] md:w-[150x] md:h-[150px]">
          <Image src={image} className="rounded" fill alt="product avatar" />
        </div>
      </div>
